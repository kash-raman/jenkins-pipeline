...
def typeToFileName = ['gradle': 'gradlew','maven':'pom.xml','npm':'package.json']

repoRegex = "$org/(.*)"
//searchedFile = typeToFileName[type]
typeToFileName.each{ type, searchedFile ->
...
void generateMavenJob(GHRepository repo){
    job("$repo.fullName") {
        description("${repo.description} - config generated on ${new Date()}")
        label('master')
        scm {
...
void generateNpmJob(GHRepository repo){
    job("$repo.fullName") {
        description("${repo.description} - config generated on ${new Date()}")
        label('master')
        scm {
